const {createDeleteReleasesPath, createDeleteTagPath, createGetReleasesPath, createTagRef} = require("./utils");
...
describe('Delete tags and releases', () => {
  ...
  it('does delete multiple releases and the tag when INPUT_DELETE_RELEASE is true', async () => {
    ...
    expect(mockFetch).toBeCalledWith(expect.objectContaining({
      method: 'DELETE',
      path: createDeleteTagPath(createTagRef(process.env.INPUT_TAG_NAME))
    }))
    expect(mockFetch).toBeCalledWith(expect.objectContaining({
      method: 'DELETE',
      path: createDeleteReleasesPath('1')
    }))
    expect(mockFetch).toBeCalledWith(expect.objectContaining({
      method: 'DELETE',
      path: createDeleteReleasesPath('2')
    }))
    expect(mockFetch).not.toBeCalledWith(expect.objectContaining({
      method: 'DELETE',
      path: createDeleteReleasesPath('3')
    }))
  })
  ...
  const createMockFetch = (options) => {
    ...
    const isDeleteReleasesMatch = (path, method) => {
      ...
      if (options.DELETE_RELEASE.error !== undefined) {
        return path.startsWith(createGetReleasesPath());
      }
      const declaredReleases = Object.keys(options.DELETE_RELEASE) ?? [];
      for (const releaseId of declaredReleases) {
        if (path === createDeleteReleasesPath(releaseId) && options.DELETE_RELEASE[releaseId] !== undefined) {
          return true;
        }
      }
      return false;
    ...
    return jest.fn(async ({path, method}) => {
      if (path === createGetReleasesPath() && method === "GET") {
      ...
      if (path === createDeleteTagPath(createTagRef(process.env.INPUT_TAG_NAME)) && method === "DELETE") {
